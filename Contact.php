<?php
// Contact.php — logged-in header (Book + Account), Contact nav active, no JS
if (session_status() === PHP_SESSION_NONE) { session_start(); }

// Simple server-side validation (digits-only phone, valid email)
$errors = [];
$values = [
  'full_name' => '',
  'phone'     => '',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Phone: digits only
  if ($values['phone'] === '' || !preg_match('/^\d+$/', $values['phone'])) {
  ...
  // (Optional) You can handle sending/storing here if no errors:
  // if (empty($errors)) { /* send email or save to DB */ }
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <title>Contact — Everest Events</title>
  ...
  <style>
    :root{
      ...
      --field-bg:#f7fbff; --btn:#10b981; --btn-ink:#062a22;
    ...
    *{box-sizing:border-box}
    ...
    /* ===== Header (same as User_Homepage logged-in) ===== */
    .header{
    ...
    .nav{
      ...
      width:100%; justify-self:stretch; justify-content:space-evenly;
      gap:clamp(24px,5vw,64px); margin:0; padding:0 12px; white-space:nowrap;
    }
    .nav a{
    ...
    .right-actions{
      justify-self:end; width:var(--right-col); display:flex; align-items:center; justify-content:space-between; gap:24px;
    }
    .toplink,
    .toplink:link,
    .toplink:visited{
    ...
    .account .dropdown{ display:none; position:absolute; top:100%; right:0; margin-top:8px; min-width:200px;
      background:#fff; border:1px solid var(--border); border-radius:10px; box-shadow:0 10px 26px rgba(20,35,57,.12); padding:8px; z-index:50;
    }
    .account:hover .dropdown, .account:focus-within .dropdown{ display:block; }
    .account:focus-within .caret{ transform:rotate(180deg); }
    .dropdown a{
      display:flex; align-items:center; gap:8px; padding:10px 12px; border-radius:8px; color:#0d1117; text-decoration:none; font-weight:600; font-size:14px;
    }
    .dropdown a:hover{ background:#f5f8fe; }

    /* ===== CONTACT CONTENT ===== */
    .contact-wrap{ max-width:1000px; margin:24px auto 48px; padding:0 20px; width:100%; }
    .contact-wrap h1{
      font-family:"Playfair Display",Georgia,serif; font-size:48px; text-align:center; margin:12px 0 26px; letter-spacing:1px;
    }
    form.contact-form{ width:100%; }
    .form-grid{ display:grid; grid-template-columns:1fr 1fr; gap:18px 22px; align-items:start; }
    label{ font-weight:600; font-size:14px; display:block; margin-bottom:6px; }
    .req{ color:#0c63ff; font-weight:800; }
    .field{
      background:var(--field-bg); border:1px solid var(--border); border-radius:10px;
      height:40px; padding:8px 10px; width:100%; font-size:14px; outline:none;
    }
    .field:focus{ border-color:#0c63ff; box-shadow:0 0 0 3px rgba(12,99,255,.12); background:#fff; }

    .error-text{ color:var(--error); font-size:12px; margin-top:6px; }
    .has-error{ border-color:var(--error) !important; background:var(--error-bg); }

    .promo{ grid-column:1 / -1; text-align:center; color:#445368; font-size:12px; line-height:1.6; margin:6px 0 8px; }

    .message-block{ grid-column:1 / -1; }
    textarea.field{ height:180px; resize:vertical; padding:10px 12px; line-height:1.5; }
    ...
    .btn-send{
      ...
      background:var(--btn); color:var(--btn-ink); border:1px solid rgba(0,0,0,.06);
      box-shadow:0 6px 14px rgba(16,185,129,.25); text-decoration:none; cursor:pointer;
    }
    ...
    @media (max-width:980px){
      ...
      .contact-wrap h1{font-size:40px}
    ...
    <form class="contact-form" action="Contact.php" method="post">
      <div class="form-grid">
        <div>
          ...
          <input class="field" id="full_name" name="full_name" type="text" placeholder="Your full name" required
                 value="<?= htmlspecialchars($values['full_name']) ?>"/>
        </div>

        <div>
          <label for="phone">Phone Number <span class="req">*</span></label>
          <input class="field <?= isset($errors['phone']) ? 'has-error' : '' ?>" id="phone" name="phone"
                 type="text" inputmode="numeric" pattern="^09\d{9}$" title="Enter an 11-digit PH mobile number starting with 09"
        ...
        <div>
          ...
          <input class="field <?= isset($errors['email']) ? 'has-error' : '' ?>" id="email" name="email"
                 type="email" placeholder="you@example.com" required
                 value="<?= htmlspecialchars($values['email']) ?>" autocomplete="email" spellcheck="false"/>
        ...
        <div>
          ...
          <input class="field" id="subject" name="subject" type="text" placeholder="How can we help?" required
                 value="<?= htmlspecialchars($values['subject']) ?>"/>
        </div>

        <p class="promo">
          Introducing the Stars of Eternity package for 200k: catering for 100, elegant cake, 100 souvenirs, flowers,
          RTW gown, bridesmaid dress, makeup, photo &amp; video, and venue options at select partner hotels.
        </p>

        <div class="message-block">
          <label for="message">Message:</label>
          <textarea class="field" id="message" name="message" placeholder="Write your message here..."><?= htmlspecialchars($values['message']) ?></textarea>
        </div>
      </div>

      <div class="actions-center">
        <button class="btn-send" type="submit" aria-label="Send message">
          <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 21l20-9L2 3l4 7 8 2-8 2-4 7z"/></svg>
          SEND
        </button>
      </div>
    </form>
      ...
      <div class="footer-col">
        ...
        <div class="socials">
          ...
          <a href="#" aria-label="Instagram" class="icon">
            <svg viewBox="0 0 24 24"><path d="M7 2h10a5 5 0 0 1 5 5v10a5 5 0 0 1-5 5H7a5 5 0 0 1-5-5V7a5 5 0 0 1 5-5zm5 3.5A5.5 5.5 0 1 1 6.5 13 5.5 5.5 0 0 1 12 7.5zm0 2A3.5 3.5 0 1 0 15.5 13 3.5 3.5 0 0 0 12 9.5zm6-2.6a1 1 0 1 1-1 1 1 1 0 0 1 1-1z"/></svg>
          </a>
        </div>
